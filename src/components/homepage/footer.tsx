...
export function Footer() {
  ...
  // Only render social icons that have a real, live URL (siteConfig.sameAs).
  // Avoids linking to 404 profiles (broken links / weak trust signal).
  ...
  const socials = siteConfig.sameAs
    .map((href) => {
      const label =
        href.includes("twitter") || href.includes("x.com")
          ...
          : href.includes("linkedin")
            ...
            : href.includes("github")
              ? "GitHub"
              : "";
      return label ? { label, href, path: socialPaths[label] } : null;
    })
    .filter((s): s is { label: string; href: string; path: string } => s !== null);
